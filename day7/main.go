...
// For part2 I had to sneak peak at another example. Got stuck for a bit too long unil
// I realized that I have to remove 1 from the result of the recursion to not count the starting bag.
...
func main() {
	...
	fmt.Println("Answer to part1:", part1(rawRules))
	fmt.Println("Answer to part2:", part2(parsedRules))
}

func part1(rawRules []string) int {
	allParentBags := getParentBags("shiny gold", &rawRules)
	allParentBags = utils.UniqueItems(allParentBags)
	return len(allParentBags)
}

func part2(parsedRules map[string][]string) int {
	return countBagsInBag("shiny gold", parsedRules) - 1
}
...
func parseRules(rules []string) map[string][]string {
	...
	for _, r := range rules {
		bagAndCondition := strings.Split(r, "bags contain")
		bag := strings.TrimSpace(bagAndCondition[0])
		conditions := parseConditions(bagAndCondition[1])
		bagRules[bag] = conditions
	}
	return bagRules
}

// Used for part2
func parseConditions(c string) []string {
	var conditions []string
	for _, cond := range strings.Split(c, ",") {
		splitCond := strings.Fields(cond)
		trimCond := splitCond[0 : len(splitCond)-1]
		joinCond := strings.TrimSpace(strings.Join(trimCond, " "))
		conditions = append(conditions, joinCond)
	}
	return conditions
}

// Used for part2
func countBagsInBag(startBag string, rules map[string][]string) int {
	var count = 1
	for _, bag := range rules[startBag] {
		if bag != "no other" {
			splitBag := strings.Fields(bag)
			numBags := utils.StringToInt(splitBag[0])
			count += numBags * countBagsInBag(strings.Join(strings.Fields(bag)[1:], " "), rules)
		}
	}
	return count
}
...
// Used for part1
func getParentBags(color string, rules *[]string) []string {
	var parentColors []string
	re := regexp.MustCompile(`^(\S+\s\S+)( bags*\scontain\s.*` + color + `.*$)`)
	for _, v := range *rules {
		r := re.FindAllStringSubmatch(v, -1)
		if r != nil {
			parentColors = append(parentColors, r[0][1])
			parentColors = append(parentColors, getParentBags(r[0][1], rules)...)
		}
	}
	return parentColors
}

var input = `dim silver bags contain 3 posh fuchsia bags.
