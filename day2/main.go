...
func main() {
	...
	for _, entry := range entries {
		e := parseEntry(entry)
		if e.validPart1() {
			validCountPart1 = 1 + validCountPart1
		}
		if e.validPart2() {
			validCountPart2 = 1 + validCountPart2
		}
	}
	fmt.Println("Part1: Found", validCountPart1, "valid password(s)")
	fmt.Println("Part2: Found", validCountPart2, "valid password(s)")
}

func (e *entry) validPart1() bool {
	o := strings.Count(e.password, e.char)
	if e.min <= o && e.max >= o {
		return true
	}
	return false
...
func (e entry) validPart2() bool {
	if xor(e.password[e.min-1], e.password[e.max-1], e.char) {
		return true
	}
	return false
}

func xor(a, b byte, char string) bool {
	if ((string(a) == char) && (string(b) != char)) || ((string(a) != char) && (string(b) == char)) {
		return true
	}
	return false
}
...
func parseEntry(input string) entry {
	...
	r := re.FindAllStringSubmatch(input, -1)
	if r == nil {
