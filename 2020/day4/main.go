...
func main() {
	countValidPart1 := 0
	countValidPart2 := 0
	rawPassport := strings.Split(input, "\n\n")
	for i := range rawPassport {
		parsedPassport := parsePassport(rawPassport[i])
		isValid := validatePassport(parsedPassport)
		if isValid == 1 {
			countValidPart1++
		}
		if isValid == 2 {
...
// parsePassport parses a passports fields and values and returns a map
func parsePassport(p string) map[string]string {
...
// validatePassport returns 1 if valid for part1, 2 if valid for part 1 and two. 0 means not valid ever.
func validatePassport(p map[string]string) int {
	reqFields := []string{
		"byr",
		"iyr",
		"eyr",
